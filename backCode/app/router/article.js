...
router.get('/',async (req,res)=>{
	...
	const list = await Article.find({},{ID:1,title:1,username:1,avatar:1,updateTIME:1,tag:1,photo_url:1,_id:0}).skip(pass).limit(parseInt(size)).sort({"updateTIME":-1})		//1:表示返回这个字段，0表示不返回
...
router.get('/all',async (req,res)=>{
	console.log(req.query)
	const list = await Article.find({title:{ $regex:req.query.title }},{ID:1,title:1,username:1,createTIME:1,updateTIME:1,tag:1,photo_url:1,_id:0}).sort({"updateTIME":-1})		//1:表示返回这个字段，0表示不返回
	const count = await Article.countDocuments()
	// console.log(count)
	// console.log(list)
	res.send({count:count,list:list})
...
router.get('/current',async (req,res)=>{
	console.log(req.query)
	const list = await Article.find({username:{ $regex:req.query.username }},{ID:1,title:1,username:1,createTIME:1,updateTIME:1,tag:1,photo_url:1,_id:0}).sort({"updateTIME":-1})		//1:表示返回这个字段，0表示不返回
	const count = await Article.countDocuments()
	// console.log(count)
	// console.log(list)
	res.send({count:count,list:list})
...
router.post('/updateArt',async (req,res)=>{
	...
	console.log(req.body)
	// const newArt = await new Article(req.body).save()
	const a =await Article.updateOne({ID:req.body.ID},{
...
router.post('/upCommentReply',async (req,res)=>{
	...
	const result = await theComment.updateOne(
		{$push:{reply:comment}}
	)
	console.log(result)
	res.send('ok')
...
router.get('/getcom',async (req,res)=>{
	console.log(req.query)
	const commentList = await Comment.find({title:{ $regex:req.query.title }})
	res.send({comments:commentList})
})
...
router.post('/delreply',async(req,res) =>{
	console.log(req.body)
	const comment = await Comment.updateOne({_id:req.body.commentid},{$pull: {"reply": { _id : req.body._id}}})
	console.log(comment)
	res.status(200).send({message:'删除成功'})
})
