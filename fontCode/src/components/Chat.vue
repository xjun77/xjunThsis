<template>
	<div>
		<el-container class="container">
			...
			<el-container style="border: 1px solid rgb(79, 87, 100);border-radius: 5px;" v-if="selectedUser">
				<el-header height="60px" class="chatheader">
					<status-icon :connected="selectedUser.connected" />{{selectedUser.username}}</el-header>
				<div class="elmainDiv" ref="msg-box">
					<el-main class="chatMain">
						<div v-for="(message, index) in selectedUser.messages" class="msgdiv">
							<div style="display: flex;justify-content: center;margin-bottom: 10px;" v-if="displaySender(message, index)">
							...
							<div :class="message.fromSelf?'msg-reflex':'msg'">
								<div>
									<el-avatar shape="square" size="medium" :src="message.fromSelf?currentUser.avatar:selectedUser.img"></el-avatar>
									<div class="time">{{message.time}}</div>
								</div>
								<div class="user-msg"><span :class="message.fromSelf?'right':'left'" v-html="message.content"></span></div>
							</div>
						</div>
					</el-main>
				</div>
				<el-footer height="100px" class="chatfooter">
					<textarea v-model.trim="input" placeholder="输入消息..." class="input" ref="sendMsg" @keyup="handleKeyCode($event)" />
