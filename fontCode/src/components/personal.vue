    ...
    <el-main class="elmain">
      ...
      <div v-show="indexItem == 2">
        ...
        <div style="margin: 5px;font-size: 15px;">
          共
          <span style="margin: 0 2px;">{{counts}}</span>篇
        </div>
      ...
      <div v-show="indexItem == 3">
        <el-form ref="FormRef2" :model="loginForm1" label-width="0" class="form_login">
          ...
          <el-form-item prop="personal">
            ...
            <el-input
              type="password"
              v-model="loginForm1.regpassword"
              placeholder="请确定密码"
              prefix-icon="el-icon-edit-outline"
            ></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submit1() {
      ...
      if (
        this.loginForm1.password.length < 6 ||
        this.loginForm1.regpassword.length < 6
      ) {
  ...
  created() {
    let that = this;
    this.$http
      .get("/user/getdetail?username=" + that.user.username)
      .then(res => {
