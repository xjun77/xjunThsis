    ...
    <el-card class="box-card" body-style="padding-bottom:12px">
      ...
      <div class="tab-card">
        ...
        <el-tag
          :key="index"
          effect="dark"
          v-for="(tag,index) in addArticle.tags"
          :type="type[index%5]"
          closable
      ...
      <el-button type="primary" class="btn" @click="uploadArt()" plain>更新资讯</el-button>
      <el-upload
        class="btn"
        action="http://upload-z2.qiniup.com"
        :before-upload="beforeUploadFM"
        :on-success="upScuccessFM"
        ref="uploadFM"
        :multiple="false"
...
export default {
  ...
  beforeCreate() {
    ...
    that.$http
      .get("/article/getArtdetail?aid=" + that.$route.query.id)
      .then(res => {
        // console.log(res);
        if (res) {
          if (res.status == 200) {
            console.log(res);
            that.addArticle.title = res.data.title;
            that.addArticle.tags = res.data.tag;
            that.addArticle.context = res.data.context;
            that.fengmianUrl = res.data.photo_url;
            that.aid = res.data.ID;
          }
        }
      })
      .catch(error => {
  ...
  methods: {
    ...
    cancleArt() {
      if (
        this.addArticle.context != "" ||
        this.addArticle.tags.length != 0 ||
